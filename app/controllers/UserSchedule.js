...
nsGridSchedule.getDay = function(timestamp, type) {
	var dateObj = nsGridSchedule.momentjs(timestamp * 1000).utcOffset('-0400');

	if (type === "day") {
		return dateObj.utcOffset('-0400').format('dddd');
	} else {
		return dateObj.utcOffset('-0400').format('h:m a');
	}
};
...
nsGridSchedule.createLayout = function(data) {
	...
	this.createBandBox = function(bName, bImage, showTime, bandId) {
		...
		var startTime = nsGridSchedule.momentjs(showTime * 1000).utcOffset('-0400').format('H.mm');
		var displayableStartTime = nsGridSchedule.momentjs(showTime * 1000).utcOffset('-0400').format('h:mm a');
...
nsGridSchedule.createDataForLayout = function(data) {
	...
	min = Math.floor(nsGridSchedule.momentjs(min * 1000).utcOffset('-0400').format('H.mm'));
	max = Math.ceil(nsGridSchedule.momentjs(max * 1000).utcOffset('-0400').format('H.mm'));
